...
class JSONConfigEditor:
    def __init__(self, root):
        self.root = root
        self.root.title("TROCKII Potions - Custom Potion Recipes [ConfigOverrideItemCraftingCosts]")
    ...
    def load_data(self):
        ...
        try:
            if os.path.exists("config.json"):
                ...
                # Update both JSON and INI views after loading config
                self.update_json_views()
        except:
            pass
    ...
    def create_widgets(self):
        ...
        notebook.add(self.potions_frame, text="Potions")
        ...
        # New tab - INI format (replaces URL JSON)
        self.ini_format_frame = ttk.Frame(notebook)
        notebook.add(self.ini_format_frame, text="INI Format")
        
        self.setup_main_tab()
        self.setup_items_tab()
        self.setup_potions_tab()
        self.setup_compressed_json_tab()
        self.setup_ini_format_tab()  # New function
    ...
    def setup_ini_format_tab(self):
        """Setup INI format tab"""
        # Header
        ttk.Label(self.ini_format_frame, text="INI Format Configuration [ConfigOverrideItemCraftingCosts]", font=('Arial', 12, 'bold')).pack(pady=10)
        
        # Text field for INI with scrollbar
        ini_frame = ttk.Frame(self.ini_format_frame)
        ini_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        
        # Scrollbar for text field
        ini_scrollbar = ttk.Scrollbar(ini_frame)
        ini_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        # Make text field non-editable
        self.ini_format_text = tk.Text(ini_frame, yscrollcommand=ini_scrollbar.set, 
                                    wrap=tk.WORD, font=('Consolas', 10), state='disabled')
        self.ini_format_text.pack(fill=tk.BOTH, expand=True)
        ini_scrollbar.config(command=self.ini_format_text.yview)
        
        # Buttons
        button_frame = ttk.Frame(self.ini_format_frame)
        button_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Button(button_frame, text="Copy to Clipboard", command=self.copy_ini_format).pack(side=tk.LEFT, padx=5)

    def update_ini_format_view(self):
        """Update INI format view"""
        try:
            ini_content = self.generate_ini_format()
            
            # Temporarily enable editing to update content
            self.ini_format_text.config(state='normal')
            self.ini_format_text.delete(1.0, tk.END)
            self.ini_format_text.insert(1.0, ini_content)
            self.ini_format_text.config(state='disabled')
        except Exception as e:
            self.ini_format_text.config(state='normal')
            self.ini_format_text.delete(1.0, tk.END)
            self.ini_format_text.insert(1.0, f"Error creating INI format: {str(e)}")
            self.ini_format_text.config(state='disabled')

    def generate_ini_format(self):
        """Generate INI format from JSON config"""
        ini_lines = []
        
        for potion_id, recipe_data in self.config_data["CustomPotionRecipes"].items():
            # Get potion display name for comment
            potion_name = self.default_potions.get(potion_id, potion_id)
            
            # Add comment with potion name
            ini_lines.append(f"; {potion_name}")
            
            # Use potion ID as-is (don't remove _C suffix)
            potion_fname = potion_id
            
            # Start building the ConfigOverrideItemCraftingCosts line
            base_line = f'ConfigOverrideItemCraftingCosts=(ItemClassString="{potion_fname}",BaseCraftingResourceRequirements=('
            
            # Add each required item
            required_items = recipe_data["RequiredItems"]
            ...
            for item in required_items:
                ...
                # Extract FName from item class path (get the last part after last dot, keep _C suffix)
                item_fname = self.extract_fname_from_class(item_class)
                
                resource_line = f'(ResourceItemTypeString="{item_fname}",BaseResourceRequirement={quantity}.0,bCraftingRequireExactResourceType=false)'
                resource_requirements.append(resource_line)
            
            # Combine all resource requirements
            resources_string = ','.join(resource_requirements)
            
            # Complete the line in one string
            complete_line = base_line + resources_string + '))'
            ini_lines.append(complete_line)
            ini_lines.append('')  # Empty line between entries
        
        return '\n'.join(ini_lines)
            
    def extract_fname_from_class(self, class_path):
        """Extract FName from class path (last part after last dot, ensure _C suffix)"""
        # Remove Blueprint' prefix and ' suffix
        clean_path = class_path.replace("Blueprint'", "").replace("'", "")
        
        # Get the last part after the last dot
        class_name = clean_path.split('.')[-1]
        
        # Add _C suffix if it's not present
        if not class_name.endswith('_C'):
            class_name += '_C'
        
        return class_name

    def copy_ini_format(self):
        """Copy INI format to clipboard"""
        # Temporarily enable to get content
        self.ini_format_text.config(state='normal')
        ini_content = self.ini_format_text.get(1.0, tk.END).strip()
        self.ini_format_text.config(state='disabled')
        
        if ini_content:
            self.root.clipboard_clear()
            self.root.clipboard_append(ini_content)
            messagebox.showinfo("Success", "INI format copied to clipboard")
    
    def setup_compressed_json_tab(self):
        """Setup readable JSON tab"""
        # Header
        ttk.Label(self.compressed_json_frame, text="Readable JSON Config (NOT FOR INI FILE!)", font=('Arial', 12, 'bold')).pack(pady=10)
    ...
    def update_url_json_view(self):
        ...
        try:
            ...
            # Encode ALL special characters including slashes
            url_encoded_json = urllib.parse.quote(compressed_json, safe='')
